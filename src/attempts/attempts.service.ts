...
export class AttemptsService {
  ...
  async submitAttempt(userId: string, dto: SubmitAttemptDto, correctCompletion: string): Promise<{ isCorrect: boolean, score: number }> {
    ...
    const score = isCorrect ? 100 : this.calculateSimilarity(submitted, correct);
  ...
  async getAllAttempts(filter: { userId?: string; isCorrect?: boolean; skip?: number; take?: number }): Promise<Attempt[]> {
    const qb = this.attemptRepo.createQueryBuilder('attempt');

    if (filter.userId) qb.andWhere('attempt.userId = :userId', { userId: filter.userId });
    if (filter.isCorrect !== undefined) qb.andWhere('attempt.isCorrect = :isCorrect', { isCorrect: filter.isCorrect });
