...
export class LeaderboardNotificationService {
  ...
  async handleRankChange(event: RankChangeEvent): Promise<void> {
    try {
      ...
      this.logger.log(
        `Sent rank change notification to user ${event.userId}: ${event.oldRank} -> ${event.newRank}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send rank change notification: ${error.message}`,
      );
    }
  }
  ...
  async handleLeaderboardReset(event: LeaderboardResetEvent): Promise<void> {
    try {
      ...
      this.logger.log(
        `Leaderboard reset: ${event.type} ${event.period} ${event.category}`,
      );
      ...
      this.logger.error(
        `Failed to handle leaderboard reset notification: ${error.message}`,
      );
    }
  }

  private createRankChangeNotification(
  ...
  private async checkMilestoneAchievements(
    ...
    for (const milestone of scoreMilestones) {
      if (
        score >= milestone &&
        (event.oldRank === null || score - milestone < 100)
      ) {
        milestones.push({
  ...
  private async sendNotification(
    ...
    this.logger.log(
      `Notification for user ${userId}: ${notification.title} - ${notification.body}`,
    );
  ...
  async sendBulkNotification(
    ...
    this.logger.log(
      `Sent bulk notification to ${userIds.length} users: ${notification.title}`,
    );
  }
